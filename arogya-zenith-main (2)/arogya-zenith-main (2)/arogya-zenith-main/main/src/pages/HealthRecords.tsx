...
const HealthRecords = () => {
  ...
  const handleAnalysis = (data: any) => {
    ...
    setTimeout(() => {
      navigate("/symptoms", { state: { analyzeData: data } });
    }, 1500);
  };
                  ...
                  prescriptions.map((p, i) => (
                    ...
                    >
                      ...
                      {p.medicine.toLowerCase().includes("surgery") && (
                        <Link to="/blood" className="flex items-center justify-between p-3 bg-destructive/10 border border-destructive/20 rounded-xl text-destructive hover:bg-destructive/20 transition-all font-bold text-[10px] uppercase tracking-tighter group/btn" title={t("records_surgery_alert")}>
                          <div className="flex items-center gap-2">
