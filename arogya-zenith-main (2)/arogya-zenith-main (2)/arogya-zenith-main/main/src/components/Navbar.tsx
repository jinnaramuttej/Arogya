      ...
      <div className="container mx-auto relative">
        <div className={`flex items-center justify-between rounded-[2rem] border px-5 py-3 backdrop-blur-xl ${navBg} ${navBorder}`}>
          ...
          <div className="hidden lg:flex items-center gap-8 flex-1 justify-center order-2">
            {[
              { name: t("nav_dashboard"), path: "/patient-dashboard" },
              { name: t("nav_symptoms"), path: "/symptoms" },
          ...
          {firstName ? (
            ...
            <Link to="/login" className={`hidden lg:inline-flex text-sm font-bold transition-colors ${navMuted} ${navHover}`}>
              Login
            </Link>
          )}
            {!loggedIn && (
              <Link to="/login" className="btn-orange hidden md:flex items-center shadow-orange-500/20">
                Get Started
              </Link>
            )}
            <button
