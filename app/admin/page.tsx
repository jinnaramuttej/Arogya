import { Users, BarChart2, Stethoscope, Hospital, HeartHandshake, UserCog } from "lucide-react";
import Link from "next/link";

const quickLinks = [
  { href: "/admin/users", label: "User Directory", description: "Search users & assign roles", icon: UserCog, color: "text-red-500", bg: "bg-red-50" },
        ...
        {quickLinks.map((item) => {
          ...
          return (
            <Link
              key={item.label}
              href={item.href}
              className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-all duration-200 group flex items-start gap-4 no-underline"
            >
              <div className={`p-3 rounded-lg ${item.bg} group-hover:scale-110 transition-transform`}>
                <Icon className={`w-6 h-6 ${item.color}`} />
              </div>
              <div>
                <h2 className="text-lg font-bold text-gray-800 group-hover:text-red-600 transition-colors">{item.label}</h2>
