    ...
    if (mode === "login") {
      ...
      if (validateSignup()) {
        ...
        if (error) {
          ...
          toast.success("Account created! Redirecting to setup Face ID...");
          if (typeof window !== "undefined") localStorage.setItem("az_user_name", name.trim());
          router.push(`/verify?mode=register&email=${encodeURIComponent(email)}&name=${encodeURIComponent(name)}`);
        }
      }
    }
    setLoading(false);
              ...
              <div className="rounded-[2rem] border border-white/20 bg-white/10 p-7 backdrop-blur-2xl shadow-2xl">
                ...
                <div className="space-y-3">
                  <button 
                    ...
                    className="w-full rounded-2xl border border-emerald-500/30 bg-emerald-500/10 py-3 text-xs font-semibold text-emerald-50 hover:bg-emerald-500/20 transition-colors flex items-center justify-center gap-3 shadow-[0_0_20px_rgba(16,185,129,0.15)] disabled:opacity-50"
                  >
